      ...
      {menu.menu?.items?.map((item) => {
        return (
          <Link
            key={item.text}
            href={item.href}
            class={`${item.href === url.url.pathname ? "current" : ""} button`}
          >
            {item.text}
          </Link>
        );
      })}
      <a class="button button--github" key={"github"} href="https://github.com/sansomguy">
      <span class="display-flex display-flex-row column-gap-xs">
        <span class="column">
